...
<div class="content view">
	...
	<h2><?=htmlspecialchars($ticket['id'], ENT_QUOTES)?> <?=htmlspecialchars($ticket['title'], ENT_QUOTES)?> <span class="<?=$ticket['status']?>">(<?=$ticket['status']?>)</span></h2>

    <div class="ticket">
        <p class="created"><?=date('F dS, G:ia', strtotime($ticket['created']))?></p>
        <p class="msg"><?=nl2br(htmlspecialchars($ticket['msg'], ENT_QUOTES))?></p>
        <p class="ticket_type"><?=nl2br(htmlspecialchars($ticket['ticket_type'], ENT_QUOTES))?></p>
        <p class="priority"><?=nl2br(htmlspecialchars($ticket['priority'], ENT_QUOTES))?></p>
    </div>
